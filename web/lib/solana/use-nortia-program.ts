...
import { NORTIA_PROGRAM_KEY, PYTH_RECEIVER_PROGRAM_KEY, TXLINE_PROGRAM_KEY } from "@/lib/solana/constants";
...
export function useProtocolStatus() {
  const { connection } = useConnection();
  const [status, setStatus] = useState({ loading: true, program: false, protocol: false, engine: false, txline: false, pyth: false });
  ...
  useEffect(() => {
    ...
    ]).then(([program, protocol, engine, txline, pyth]) => {
      if (active) setStatus({ loading: false, program: Boolean(program?.executable), protocol: Boolean(protocol), engine: Boolean(engine), txline: Boolean(txline?.executable), pyth: Boolean(pyth?.executable) });
    }).catch(() => {
      if (active) setStatus({ loading: false, program: false, protocol: false, engine: false, txline: false, pyth: false });
    });
    return () => { active = false; };
  }, [connection]);
