...
export function OptimisticResolutionPanel({ market }: { market: Market }) {
  ...
  return (
    <section className="optimistic-panel" id="optimistic-resolution">
      ...
      {claimable > 0n && <div className="permissionless-finalize"><div><CircleDollarSign size={16} /><span><strong>{formatUsdc(claimable)} USDC bond payout</strong><small>The finalized proposal records this wallet as an entitled claimant.</small></span></div><button type="button" disabled={busy} onClick={() => void claim()}>{actionText ?? "Claim bond payout"}</button></div>}
