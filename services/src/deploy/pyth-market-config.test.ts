...
test("Pyth observation accepts the exact twenty-minute boundary", () => {
...
test("Pyth observation rejects missing, invalid, and early timestamps", () => {
  const now = Date.parse("2026-07-20T10:00:00Z") / 1_000;
...
test("Pyth market ID defaults deterministically and accepts unsigned 64-bit bounds", () => {
...
test("Pyth market ID rejects zero, overflow, negative, and malformed values", () => {
