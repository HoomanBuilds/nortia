  ...
  for (;;) {
    ...
    for (const { publicKey: marketAddress, account } of markets) {
      ...
      if (action === "begin-refund") {
        const signature = await program.methods.beginRefund().accountsPartial({ market: marketAddress }).rpc();
        log({ event: "refunds-opened", signature, ...context });
        continue;
      }
      if (!txline) {
        log({ event: "resolution-blocked", reason: "TxLINE credentials are not configured", ...context });
        continue;
      }
      ...
      if (!finalRecord) {
        log({ event: "resolution-waiting", reason: "No game_finalised record", fixtureId, ...context });
        continue;
      }
      const seq = Number(finalRecord.seq ?? finalRecord.Seq);
      const proof = await txline.validationProof(fixtureId, seq, [account.scoreKeyA, account.scoreKeyB]);
      ...
      const signature = await program.methods.resolveMarket(seq, payload).accountsPartial({
